...
const RallyCard: React.FC<RallyCardProps> = React.memo(function RallyCard({ rally }) {
  return (
    <div className="bg-gradient-to-br from-pink-900 via-gray-900 to-pink-800 shadow-xl rounded-2xl p-6 hover:scale-105 hover:shadow-2xl transition-transform duration-300 ease-in-out">
      ...
      <h3 className="text-3xl font-extrabold text-pink-400 mb-2 animate-fadeInUp">{rally.nombre}</h3>
      <p className="text-lg text-gray-200 mb-4 animate-fadeInUp delay-100">{rally.descripcion}</p>
      <Link
        to={`/rallies/${rally.id}`}
        ...
        aria-label={`Ver publicaciones de la galería ${rally.nombre}`}
      >
        Ver publicaciones
      </Link>
    </div>
