  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      if (!/\S+@\S+\.\S+/.test(email) || email.length > 255) {
        setErrorMessage(
          "Por favor, ingresa un email válido y que no exceda los 255 caracteres."
        );
        setIsLoading(false);
        return;
      }
      if (
        ...
        setErrorMessage(
          "La contraseña debe tener entre 6 y 255 caracteres, incluir mayúsculas, minúsculas y números."
        );
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        if (
          apiMsg === "El usuario ya está registrado" ||
          apiMsg === "El email ya está registrado"
        ) {
          ...
          setErrorMessage(
            apiMsg || "Ocurrió un error al registrarse. Inténtalo más tarde."
          );
        }
      } finally {
        setIsLoading(false);
    ...
    [setErrorMessage, setSuccessMessage, setIsLoading]
  );
